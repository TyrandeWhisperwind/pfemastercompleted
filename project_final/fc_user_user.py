...
def fc_user_user(usage_matrix, distance_matrix, test_file, threshold=0.43):
...
def main():
    ...
    print("#FC user user")
    results.update({'fc_user_user': []})
    usage_matrix = data.get_usage_matrix()
    distance_matrix = load_data('./distance_matrices/fc_uu_dist_mat.csv')
    ...
    t = vmin
    while t <= vmax:
        mae, rmse, pres, rec = fc_user_user(usage_matrix, distance_matrix, "/home/imad/Desktop/PFE/db/ml-100k/ua.test", threshold=t)
        print(t)
        results['fc_user_user'].append({'threshold': t, 'mae': mae, 'rmse': rmse, 'pres': pres, 'rec': rec})
        t = t + step
    

    print("#FC user user | mean user")
    ...
    t = vmin
    while t <= vmax:
        mae, rmse, prec, rec = fc_user_user(usage_matrix, distance_matrix, "/home/imad/Desktop/PFE/db/ml-100k/ua.test", threshold=t)
        print(t)
        results['fc_user_user_ua'].append({'threshold': t, 'mae': mae, 'rmse': rmse, 'pres': prec, 'rec': rec})
        t = t + step

    
    print("#FC user user | mean item")
    results.update({'fc_user_user_ia': []})
    usage_matrix = mean(data.get_usage_matrix(), axis=0)
    distance_matrix = load_data('./distance_matrices/fc_uu_ia_dist_mat.csv')
    ...
    while t <= vmax:
        mae, rmse, prec, rec = fc_user_user(usage_matrix, distance_matrix, "/home/imad/Desktop/PFE/db/ml-100k/ua.test", threshold=t)
        print(t)
        results['fc_user_user_ia'].append({'threshold': t, 'mae': mae, 'rmse': rmse, 'pres': prec, 'rec': rec})
        t = t + step


    print("#FC user user | svd")
    ...
    while t <= vmax:
        mae, rmse, prec, rec = fc_user_user(usage_matrix, distance_matrix, "/home/imad/Desktop/PFE/db/ml-100k/ua.test", threshold=t)
        print(t)
        results['fc_user_user_svd'].append({'threshold': t, 'mae': mae, 'rmse': rmse, 'pres': prec, 'rec': rec})
        t = t + step

    '''
    print("#FC user user | GNB")
    usage_matrix = gnb_uu_100k(data.get_usage_matrix())
    distance_matrix = load_data('./distance_matrices/fc_uu_gnb_dist_mat.csv')
    mae, rmse = fc_user_user(usage_matrix, distance_matrix, "/home/imad/Desktop/PFE/db/ml-100k/ua.test", threshold=t)
    results.update({'fc_user_user_gnb': {'threshold': t, 'mae': mae, 'rmse': rmse}})
    '''
    json.dump(results, open('./results/fc_user_user.json', 'w'))
